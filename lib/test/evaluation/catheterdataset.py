...
class CatheterDataset(BaseDataset):
    ...
    def _get_sequence_path(self,seq_id,training_mode="Val"):
        seq_name = self.sequence_list[seq_id-50]
        class_name = seq_name.split('-')[0]
        vid_id = seq_name.split('-')[1]

        return os.path.join(self.base_path, training_mode, class_name + '-' + vid_id)
    ...
    def _construct_sequence(self,sequence_name):
        ...
        sequence_number = sequence_name.split('-')[1]
        ...
        full_occlusion = np.zeros(ground_truth_rect.shape[0])

        out_of_view = np.zeros(ground_truth_rect.shape[0])
...
if "__main__" == __name__:
    ...
    sequence = dataset._construct_sequence(sequence_name='Catheter-55')
    # print(sequence)
